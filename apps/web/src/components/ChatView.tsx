...
import { buildSshWorktreePath } from "../sshExecutionPath";
import {
...
function buildLocalDraftThread(
  ...
  projectExecutionTarget: Thread["executionTarget"],
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const noopRemoveTerminalContext = useCallback((_contextId: string) => {}, []);
  const noopAddTerminalContext = useCallback(() => {}, []);
    ...
    () =>
      draftThread
        ? buildLocalDraftThread(
            threadId,
            draftThread,
            resolveDraftExecutionTarget({ project: fallbackDraftProject }),
  ...
  const activeProjectExecutionCwd = activeProject
    ? resolveProjectExecutionRoot(activeProject)
    : null;
  ...
  const selectedEffort: CodexReasoningEffort | null =
    selectedProvider === "codex"
      ? ((composerDraft.effort ?? getDefaultReasoningEffort("codex")) as CodexReasoningEffort)
      : null;
  const selectedCodexFastModeEnabled =
    selectedProvider === "codex" ? composerDraft.codexFastMode === true : false;
  const codexReasoningOptions =
    selectedProvider === "codex"
      ? (reasoningOptions as ReadonlyArray<CodexReasoningEffort>)
      : ([] as const);
  ...
  const gitCwd = resolveExecutionCwd({
  ...
  const threadTerminalRuntimeEnv = useMemo(() => {
    if (!activeProjectExecutionCwd) return {};
    return projectScriptRuntimeEnv({
      project: {
        cwd: activeProjectExecutionCwd,
      },
      worktreePath: activeThreadWorktreePath,
    });
  }, [activeProjectExecutionCwd, activeThreadWorktreePath]);
  ...
  const runProjectScript = useCallback(
    async (
      ...
      const targetCwd = options?.cwd ?? gitCwd ?? activeProjectExecutionCwd ?? activeProject.cwd;
      ...
      const runtimeEnv = projectScriptRuntimeEnv({
        project: {
          cwd: activeProjectExecutionCwd ?? activeProject.cwd,
        },
    ...
    await (async () => {
      ...
      if (baseBranchForWorktree) {
        ...
        const result = await createWorktreeMutation.mutateAsync({
          ...
          newBranch,
          ...(activeProject.executionTarget.kind === "ssh"
            ? { path: buildSshWorktreePath(activeProject.executionTarget, newBranch) }
            : {}),
        });
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          >
            <MessagesTimeline
              ...
              workspaceRoot={activeProjectExecutionCwd ?? undefined}
            />
          </div>
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:min-w-max sm:overflow-visible">
                      ...
                      {selectedProvider === "codex" && selectedEffort != null ? (
                        <>
                          ...
                          />
                          <CodexTraitsPicker
                            effort={selectedEffort ?? getDefaultReasoningEffort("codex")}
      ...
      {(() => {
        ...
        return (
          <ThreadTerminalDrawer
            ...
            onAddTerminalContext={noopAddTerminalContext}
          />
        );
      })()}
...
const ProviderHealthBanner = memo(function ProviderHealthBanner({
  ...
  const defaultMessage =
    status.status === "error"
      ? `${providerDisplayName(status.provider)} provider is unavailable.`
      : `${providerDisplayName(status.provider)} provider has limited availability.`;

  return (
    <div className="pt-3 mx-auto max-w-3xl">
      <Alert variant={status.status === "error" ? "error" : "warning"}>
        <CircleAlertIcon />
        <AlertTitle>{`${providerDisplayName(status.provider)} provider status`}</AlertTitle>
...
function providerDisplayName(provider: ProviderPickerKind): string {
  switch (provider) {
    case "codex":
      return "Codex";
    case "claudeCode":
