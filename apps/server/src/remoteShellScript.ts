...
export const REMOTE_SHELL_SCRIPT_FALLBACK = `#!/usr/bin/env bash
set -euo pipefail
...
fail_remote_execution() {
  local detail="$1"
  printf '[t3] %s\n' "\${detail:-Remote execution target is unavailable.}" >&2
  exit 1
}
...
export const REMOTE_PROVIDER_SCRIPT_FALLBACK = `#!/usr/bin/env bash
set -euo pipefail
...
REMOTE_USER="\${T3_REMOTE_USER:-}"
REMOTE_PORT="\${T3_REMOTE_PORT:-}"
REMOTE_PROVIDER_BIN="\${T3_REMOTE_PROVIDER_BIN:-}"
REMOTE_PROVIDER_HOME="\${T3_REMOTE_PROVIDER_HOME:-}"
STATE_DIR="\${T3_REMOTE_STATE_DIR:-\${HOME}/.t3code/remote-execution}"
...
check_remote() {
  if [[ -z "\${REMOTE_HOST}" || -z "\${REMOTE_PATH}" || -z "\${REMOTE_PROVIDER_BIN}" ]]; then
    return 1
  fi
  ssh "\${SSH_OPTS[@]}" "\${REMOTE_TARGET}" "exit 0" >/dev/null 2>&1
}
...
fail_remote_execution() {
  local detail="\${1:-Remote provider target is unavailable.}"
  printf '[t3] %s\n' "\${detail}" >&2
  exit 1
}
...
main() {
  if ! check_remote; then
    fail_remote_execution "SSH target \${REMOTE_TARGET} is unavailable for provider execution."
  fi

  local remote_cwd
  remote_cwd="$(replace_paths_to_remote "$(pwd -P)")"
  local remote_command=("\${REMOTE_PROVIDER_BIN}")
  for arg in "$@"; do
    remote_command+=("$(replace_paths_to_remote "\${arg}")")
  done

  local remote_prefix=""
  if [[ -n "\${REMOTE_PROVIDER_HOME}" ]]; then
    remote_prefix+="export CODEX_HOME=$(printf '%q' "\${REMOTE_PROVIDER_HOME}"); "
  fi

  local quoted_command
  quoted_command="$(printf '%q ' "\${remote_command[@]}")"

  exec ssh "\${SSH_OPTS[@]}" "\${REMOTE_TARGET}" \
    "cd '$(printf '%s' "\${remote_cwd}")' 2>/dev/null || cd '$(printf '%s' "\${REMOTE_PATH}")'; \${remote_prefix}exec \${quoted_command}"
}
