...
> {
  ...
  const tlsConfig = yield* resolveServerTlsConfig({
    ...
    Effect.mapError((cause) => new ServerLifecycleError({ operation: "resolveServerTlsConfig", cause })),
  );
  ...
  const workspaceAccessTokenPath = path.join(serverConfig.stateDir, WORKSPACE_ACCESS_TOKEN_FILENAME);
  ...
  const resolvePersistedWorkspaceAccessToken = Effect.fnUntraced(function* () {
    ...
    yield* fileSystem.makeDirectory(serverConfig.stateDir, { recursive: true }).pipe(
  ...
  networkServer = (currentTlsOptions
    ? https.createServer(currentTlsOptions, handleHttpRequest)
    : http.createServer(handleHttpRequest)) as http.Server | https.Server;
