...
import {
  REMOTE_PROVIDER_SCRIPT_FALLBACK,
  REMOTE_SHELL_SCRIPT_FALLBACK,
} from "./remoteShellScript.ts";

const REMOTE_EXECUTION_DIR = "remote-execution";
const REMOTE_SHELL_SCRIPT_NAME = "t3-remote-shell.sh";
const REMOTE_PROVIDER_SCRIPT_NAME = "t3-remote-provider.sh";
...
const REMOTE_PROVIDER_SCRIPT = (() => {
  try {
    return fs.readFileSync(new URL("./remote-provider.sh", import.meta.url), "utf8");
  } catch {
    return REMOTE_PROVIDER_SCRIPT_FALLBACK;
  }
})();
...
export function ensureRemoteProviderScript(stateDir: string): string {
  const scriptPath = path.join(stateDir, REMOTE_EXECUTION_DIR, REMOTE_PROVIDER_SCRIPT_NAME);
  ensureExecutableFile(scriptPath, REMOTE_PROVIDER_SCRIPT);
  return scriptPath;
}
...
export function resolveRemoteProviderCwd(
  cwd: string | undefined,
  shellEnvironment: Record<string, string> | undefined,
): string | undefined {
  if (!cwd || !shellEnvironment) {
    return cwd;
  }
  const localPath = shellEnvironment.T3_LOCAL_PATH?.trim();
  const remotePath = shellEnvironment.T3_REMOTE_PATH?.trim();
  const remoteProviderBinary = shellEnvironment.T3_REMOTE_PROVIDER_BIN?.trim();
  if (!localPath || !remotePath || !remoteProviderBinary) {
    return cwd;
  }
  return cwd.startsWith(localPath) ? `${remotePath}${cwd.slice(localPath.length)}` : cwd;
}

function remoteProviderBinaryName(provider: ProviderDriverKind): string {
...
export function buildRemoteProviderOptions(input: {
  ...
  const scriptPath = ensureRemoteProviderScript(input.stateDir);
  ...
  if (input.provider === "claudeAgent") {
    return {
      claudeAgent: {
        ...input.baseProviderOptions?.claudeAgent,
        binaryPath: scriptPath,
        shellEnvironment: mergeShellEnvironment(
          input.baseProviderOptions?.claudeAgent?.shellEnvironment,
          {
            ...shellEnvironment,
            T3_REMOTE_PROVIDER_BIN:
              input.baseProviderOptions?.claudeAgent?.binaryPath ??
              remoteProviderBinaryName(input.provider),
          },
        ),
      },
    };
  ...
  return {
    codex: {
      ...input.baseProviderOptions?.codex,
      binaryPath: scriptPath,
      shellEnvironment: mergeShellEnvironment(input.baseProviderOptions?.codex?.shellEnvironment, {
        ...shellEnvironment,
        T3_REMOTE_PROVIDER_BIN:
          input.baseProviderOptions?.codex?.binaryPath ?? remoteProviderBinaryName(input.provider),
        ...(input.baseProviderOptions?.codex?.homePath
          ? { T3_REMOTE_PROVIDER_HOME: input.baseProviderOptions.codex.homePath }
          : {}),
      }),
    },
  };
