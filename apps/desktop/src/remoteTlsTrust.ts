      ...
      `${JSON.stringify(
        {
          ...
          hosts: Object.fromEntries(
            [...hosts.entries()].map(([hostname, fingerprints]) => [
              hostname,
              [...fingerprints].toSorted(),
            ]),
          ),
        } satisfies TrustedRemoteCertificateStore,
        null,
...
export function createRemoteTlsTrustController(stateDir: string): {
  ...
  const isTrustedByPinnedFingerprint = (
    hostname: string,
      ...
      const socket = TLS.connect(
        ...
        () => {
          try {
            ...
            const subjectName =
              typeof certificate.subject?.CN === "string" ? certificate.subject.CN : null;
            const issuerName =
              typeof certificate.issuer?.CN === "string" ? certificate.issuer.CN : null;
