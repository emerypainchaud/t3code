...
async function probeRemoteWorkspace(input: {
  ...
  try {
    ...
    return {
      ...
      lingerEnabled: typeof parsed.lingerEnabled === "boolean" ? parsed.lingerEnabled : null,
      deployedVersion:
...
function capabilityWarnings(capabilities: RemoteWorkspaceCapabilities): string[] {
  ...
  if (!capabilities.claudeCode) {
    warnings.push(
      "Remote host is missing `claude`/`claude-code`, so Claude Code sessions cannot start there yet.",
    );
  }
  return warnings;
}
