...
import { ServerProviderStatus } from "./server.ts";
...
export const ProjectListDirectoryInput = Schema.Struct({
  ...
  limit: PositiveInt.check(Schema.isLessThanOrEqualTo(PROJECT_LIST_DIRECTORY_MAX_ENTRIES)).pipe(
    Schema.withDecodingDefault(Effect.succeed(PROJECT_LIST_DIRECTORY_MAX_ENTRIES)),
  ),
});
...
export const ProjectSshPreflightInput = Schema.Struct({
  ...
  ignores: Schema.Array(TrimmedNonEmptyString).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
});
...
export const ProjectSshPreflightResult = Schema.Struct({
  ...
  providers: Schema.Array(ServerProviderStatus),
